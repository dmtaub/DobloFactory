...
function NB_RADIUS(SCALE)  = (SCALE < 0.6) ? (4.7 / 2 * LEGO_SCALE(SCALE)) : (9.3 / 2.0 * SCALE);    // radius Lego vs. DUPLO
...
// Must be adjusted with respect to layer resolution and other slicing considerations
function NB_BOTTOM_RADIUS(SCALE)        = (SCALE < 0.6) ? 6.6/2*LEGO_SCALE(SCALE) : 13.45/2*SCALE; //was 13.4
...
function DOBLOWALL(SCALE) = (SCALE < 0.6) ? 1.1 * LEGO_SCALE(SCALE): 1.55 *SCALE; // Lego vs. Duplo, Lego is not 2x smaller

function USE_INSET(SCALE) = true;//(SCALE < 0.6) ? false : true;
function INSET_WIDTH(SCALE)    = (SCALE < 0.6) ? 0.6 *LEGO_SCALE(SCALE) : 1.50 * SCALE; //little inset walls to make it stick
